...
const Navbar = ({ navbaritems }: NavbarProps) => {
  ...
  return (
    <nav className="bg-white border-gray-200 pt-5 pb-3">
          ...
          <ul className="font-medium flex items-center flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white">
            ...
            {navbaritems[0].buttonItems.map((button) => (
              <li key={button.btnName}>
                <Link
                  href={button.btnLink}
                  className="contactBtn text-black text-lg font-medium px-8 py-[10px] md:px-10 md:py-[12px] rounded-full border border-transparent hover:bg-transparent hover:border hover:border-black"
                  aria-current="page"
                >
                  {button.btnName}
                </Link>
