...
function ContentFilter({
  ...
  const [filter, setFilter] = useState<"popular" | "latest" | "all">("all");

  const handleFilterChange = (value: "popular" | "latest" | "all") => {
    setFilter(value);
  };
  useEffect(() => {
    ...
    if (filter === "popular") {
      filteredBlogs = blogs.filter(
        (blog) => blog.category.toLowerCase() === "popular"
      );
    } else if (filter === "latest") {
      filteredBlogs = blogs.filter(
        (blog) => blog.category.toLowerCase() === "latest"
      );
    }
    if (searchTerm) {
  ...
  }, [filter, searchTerm, blogs, setFilteredBlogs]);
    ...
    <div className="pl-32 pr-32 mt-16 flex items-start lg:items-center flex-col lg:flex-row gap-8 justify-between mb-[60px] md:mb-[70px] lg:mb-[80px] xl:mb-[88px] overflow-hidden">
      {contentfilter.filterButton && (
        <div className="flex gap-4 whitespace-nowrap overflow-x-auto w-full custom-scrollbar">
          {["all", "latest", "popular"].map((button) => (
            <div
              key={button}
              onClick={() =>
                handleFilterChange(button as "all" | "latest" | "popular")
              }
              className={`cursor-pointer px-[24px] sm:px-[30px] lg:px-[40px] py-[13px] text-sm sm:text-base font-semibold rounded-full text-black transition leading-[1] flex items-center justify-center 
            ${filter === button ? "bg-theme-green" : "bg-gray-100 hover:bg-gray-200"}`}
            >
